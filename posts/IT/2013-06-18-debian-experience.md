...
Там я обнаружил, что отстутствует строчка
...
поскольку start-stop-daemon не использует pam. Сам start-stop-daemon - это debian-специфичная вещь, представляет собой скрипт для запуска демонов, изначально написаный на Perl, а затем
переписаный на C для ускорения. Что интересно, в RHEL6 и производных тоже используется свой
...
Также предлагались подобные патчи, основаные на инициализации сессии pam, и каждый раз отклонялись.

##Поломаный update-rc.d start/stop/defaults
...
У меня, к стастью, проявился только на некотором железе, которое пришло под новый проект,
...
Тут, на мой взгляд, единственный конкурент Debain - это RHEL и его производные.
