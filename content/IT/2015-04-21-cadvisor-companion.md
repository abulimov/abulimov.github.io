...
я это сделал достаточно легко - обернул их в простенький wrapper на bash,
...
А при проблемах вывод этого wrapperа получается такой:
...
- утилиты top, ps и прочие, получающие данные из /proc, показывают в контейнерах
[неверные данные](http://fabiokung.com/2014/03/13/memory-inside-linux-containers/);
- cAdvisor не предоставляет информации о процессах внутри контейнера.

Поскольку информацию о использовании памяти/CPU я беру по HTTP из
...
В общем, задача оказалась не сложной - достаточно всего лишь пройтись по
файловой системе `/proc` на родительском хосте контейнеров, собрать данные
...
Подробнее о том, что из себя представляет API, почему я не стал использовать
Docker API, и прочие полезные вещи можно почитать на
...
из начала статьи, которое и побудило меня к написанию своего сервиса, выглядит примерно так:
...
65534  1285  18.3   0.0    18396                     1632      S /usr/sbin/nutcracker -v 11 -c /etc/twemproxy/config.yml
 ...
 | cpu_usage=83.32%;80;90;0;100
```

Проверки эти я делаю все [тем же скриптом check_cadvisor.py](https://github.com/abulimov/utils/blob/master/nagios/check_cadvisor.py),
